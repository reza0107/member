# Aplikasi Web Sistem Absensi perusahaan Berbasis `QR Code` <br> (+ Dukungan RFID)

<a href="https://github.com/ikhsan3adi/absensi-perusahaan-qr-code/actions/workflows/ci.yml">
    <img src="https://img.shields.io/github/actions/workflow/status/ikhsan3adi/absensi-perusahaan-qr-code/ci.yml?branch=master&style=for-the-badge&label=Continuous%20Integration&labelColor=%239f39b5&logo=github" alt="Continuous Integration">
</a>
<a href="https://github.com/ikhsan3adi/absensi-perusahaan-qr-code/stargazers">
    <img src="https://img.shields.io/github/stars/ikhsan3adi/absensi-perusahaan-qr-code?style=for-the-badge&labelColor=%239f39b5&color=%2300bcd4&logo=github" alt="GitHub Repo stars">
</a>
<a href="https://github.com/ikhsan3adi/absensi-perusahaan-qr-code/graphs/contributors">
    <img src="https://img.shields.io/github/contributors-anon/ikhsan3adi/absensi-perusahaan-qr-code?style=for-the-badge&labelColor=%239f39b5&color=%2300bcd4&logo=github" alt="GitHub Contributors">
</a>
<a href="https://github.com/ikhsan3adi/absensi-perusahaan-qr-code/network/members">
    <img src="https://img.shields.io/github/forks/ikhsan3adi/absensi-perusahaan-qr-code?style=for-the-badge&labelColor=%239f39b5&color=%2300bcd4&logo=github" alt="GitHub forks">
</a>
<a href="https://github.com/ikhsan3adi/absensi-perusahaan-qr-code/watchers">
    <img src="https://img.shields.io/github/watchers/ikhsan3adi/absensi-perusahaan-qr-code?style=for-the-badge&labelColor=%239f39b5&color=%2300bcd4&logo=github" alt="GitHub watchers">
</a>
...
Aplikasi Web Sistem Absensi perusahaan adalah sebuah proyek yang bertujuan untuk mengotomatisasi proses absensi di lingkungan perusahaan. Aplikasi ini dikembangkan dengan menggunakan framework CodeIgniter 4 dan didesain untuk mempermudah pengelolaan dan pencatatan kehadiran siswa dan member.

> [Instalasi & Cara Penggunaan](#cara-penggunaan)

## Fitur Utama

- **QR Code scanner.** Setiap siswa/member menunjukkan qr code kepada perangkat yang dilengkapi dengan kamera. Aplikasi akan memvalidasi QR code dan mencatat kehadiran siswa ke dalam database.
- **RFID Integration.** Dukungan presensi menggunakan RFID card sebagai alternatif QR Code. Setiap siswa/member dapat menggunakan RFID card untuk melakukan presensi dengan validasi kode RFID yang unik.
...
- **Login petugas.**
- **Dashboard petugas.** Petugas perusahaan dapat dengan mudah memantau kehadiran siswa dalam periode waktu tertentu melalui tampilan yang disediakan.
- **Dashboard Wali Kelas.** Fitur khusus untuk member wali kelas dengan kemampuan:
...
- **QR Code generator & downloader.** Petugas yang sudah login akan men-generate dan/atau mendownload qr code setiap siswa/member. Setiap siswa akan diberikan QR code unik yang terkait dengan identitas siswa. QR code ini akan digunakan saat proses absensi.
- **Ubah data absen siswa/member.** Petugas dapat mengubah data absensi setiap siswa/member. Misalnya mengubah data kehadiran dari `tanpa keterangan` menjadi `sakit` atau `izin`.
- **Tambah, Ubah, Hapus(CRUD) data siswa/member.**
...
| <img src="./screenshots/notif-wa.png" width="320px" alt="Notifikasi WA"> <br> Notifikasi otomatis dikirim ke nomor HP siswa/member setelah berhasil presensi. |  |

---

### Dashboard Admin

|  ![Dashboard Admin](./screenshots/admin-dashboard-1.9.10.png) <br> Dashboard Utama   |                         ![Absen Siswa](./screenshots/absen-siswa-1.9.10.png) <br> Absensi Siswa                         |        ![Absen member](./screenshots/absen-member-1.9.10.png) <br> Absensi member        |
| :----------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------: |
|     ![Generate Laporan](./screenshots/laporan-1.9.10.png) <br> Generate Laporan      |                         ![CRUD Data Siswa](./screenshots/data-siswa-1.9.10.png) <br> Data Siswa                         |        ![CRUD Data member](./screenshots/data-member-1.9.10.png) <br> Data member        |
...
| :----------------------------------------------------------------------------------: | :--------------------------------------------------------------------------: |
| Dashboard khusus untuk member wali kelas dengan statistik kehadiran siswa di kelasnya. | Wali kelas dapat melihat dan mengelola kehadiran siswa di kelas yang diampu. |
...
```bash
git clone https://github.com/ikhsan3adi/absensi-perusahaan-qr-code.git
cd absensi-perusahaan-qr-code
```
...
- `tb_siswa` - Data siswa
- `tb_presensi_member` - Presensi member
- `tb_presensi_siswa` - Presensi siswa
- `users` - Akun pengguna (ditambah kolom `is_superadmin`, `id_member`)
...
- **Kepsek**
  - Kepala perusahaan dengan akses terbatas*
  - Dapat mengakses fitur generate laporan
...
\* Akses dapat berubah jika dihubungkan ke data member / di-assign sebagai wali kelas

</details>

### Cara Menggunakan Akun Wali Kelas

Fitur Wali Kelas memungkinkan member untuk mengelola kehadiran siswa di kelas yang diampu. Berikut cara mengaktifkan dan menggunakan fitur ini:

#### Persiapan Data

Pastikan data berikut sudah tersedia:

- **Data member** sudah terdaftar di sistem
- **Data Kelas** sudah dibuat

#### Menautkan member sebagai Wali Kelas
...
4. Pada dropdown **"Wali Kelas"**, pilih nama member yang akan menjadi wali kelas
5. Klik **Simpan**
...
3. Untuk membuat akun baru:
   ...
   - Pada dropdown **"Hubungkan ke member"**, pilih nama member
   - Klik **Simpan**
...
2. Login menggunakan **username dan password** yang sudah dibuat/dihubungkan ke data member
3. Setelah login, sistem akan otomatis mendeteksi bahwa akun tersebut terhubung dengan data member yang menjadi wali kelas
...
- Satu akun petugas hanya bisa dihubungkan ke **satu data member**
- member yang belum ditugaskan sebagai wali kelas tidak akan melihat menu Dashboard Wali Kelas
...
- **Import Data Siswa** - Buka menu Data Siswa > Import CSV. Download template [csv_member_template.csv](./public/assets/file/csv_siswa_template.csv) dan upload setelah diisi. Contoh [csv_siswa_example.csv](./public/assets/file/csv_siswa_example.csv)
  > Pastikan id kelas cocok dengan data kelas yang ada di database

- **Import Data member** - Buka menu Data member > Import CSV. Download template [csv_member_template.csv](./public/assets/file/csv_member_template.csv) dan upload setelah diisi.
  > Perhatikan untuk impor data member & siswa:
  >
  > ```csv
  > ...,jenis_kelamin,... <- kolom
...
>
> - Untuk mengubah konfigurasi nama perusahaan, tahun ajaran logo perusahaan dll sudah disediakan pengaturan (khusus untuk superadmin).
> - Logo perusahaan Rekomendasi 100x100px atau 1:1 dan berformat PNG/JPG.
...
Dengan aplikasi web sistem absensi perusahaan berbasis QR code ini, diharapkan proses absensi di perusahaan menjadi lebih efisien dan terotomatisasi. Proyek ini dapat diadaptasi dan dikembangkan lebih lanjut sesuai dengan kebutuhan dan persyaratan perusahaan Anda.
...
<a href="https://www.star-history.com/#ikhsan3adi/absensi-perusahaan-qr-code&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=ikhsan3adi/absensi-perusahaan-qr-code&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=ikhsan3adi/absensi-perusahaan-qr-code&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=ikhsan3adi/absensi-perusahaan-qr-code&type=Date" />
 </picture>
</a>
